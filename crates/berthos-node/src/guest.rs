use berthos_protocol::{ActionBatch, Frame, LeaseRequest};
...
#[cfg(test)]
mod stub {
    use berthos_protocol::{Ack, AckKind, AckResult, Action, ActionBatch, Frame, FrameKind};
    ...
    impl StubSession {
        pub(crate) fn start(req: &berthos_protocol::LeaseRequest) -> Result<Self> {
            req.validate_mvp()?;
            Ok(Self {
                session_id: new_id("s"),
