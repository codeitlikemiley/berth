...
async fn object_store_round_trips_through_mnt_s3() {
    ...
    // Wait for MinIO, then seed a file through the same rclone the node uses.
    let mut ready = false;
    for _ in 0..60 {
    ...
    let ws = unique_ws();
    let mut req = lease(&ws);
    req.object = Some(berth_protocol::ObjectStore {
    ...
    let _ = Command::new("docker")
        .args(["volume", "rm", "-f", &berth_node::volume_name(&ws)])
        .status();
    let _ = Command::new("docker")
        .args(["volume", "rm", "-f", &berth_node::s3_volume_name(&ws)])
        .status();
}
