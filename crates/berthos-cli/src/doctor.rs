...
pub(crate) fn run_doctor(home: &Path) -> Result<String> {
    let image = berthos_node::image_from_env();
...
fn probe_image(name: &str) -> std::result::Result<String, String> {
    ...
    match String::from_utf8_lossy(&out.stdout).trim() {
        v if v == berthos_node::EGRESS_VERSION => Ok(format!("image {name} (egress v{v})")),
        "" | "<no value>" => Err(format!(
